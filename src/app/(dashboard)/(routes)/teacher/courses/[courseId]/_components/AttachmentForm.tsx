...
import { ImageIcon, PenLine, PlusCircle, File, X, XCircle, Loader2 } from 'lucide-react';
            ...
            <Card className='w-full'>
                <div className='bg-card rounded-md p-4'>
                    ...
                    <div className="form mt-2">
                        {
                            !isEditing && (attachments.length === 0 && <p className='text-sm mt-2'>No attachments yet</p>)
                        }
                        {
                            !isEditing && (attachments.length > 0 && <div className='mt-4'>
                                <div className="flex flex-col gap-3">
                                    {
                                        attachments.map((attach, index) => {
                                            return <div className='p-2 bg-muted rounded-lg flex justify-between' key={index}>
                                                <div className="flex items-center gap-4"> <File />
                                                    {attach.name}</div>
                                                {deleteId === attach.id && <Loader2 />}
                                                {
                                                    deleteId !== attach.id && <XCircle onClick={() => {
